...
template <typename T_, int kTileM_ = 128, int kTileN_ = 128, int kTileK_ = 32,
          int kStage_ = 5, int kSmemLayoutCBatch_ = 1,
...
//   using mma_op = SM80_16x8x16_F16F16F16F16_TN;
  using mma_op = SM80_16x8x32_F32E4M3E4M3F32_TN;
  ...
  using G2SCopyA =
      decltype(make_tiled_copy(g2s_copy_atom{},
                               ...
                               make_layout(make_shape(Int<1>{}, Int<16>{})))); //TiledNumVal{}=16
  using G2SCopyB = G2SCopyA;

  // shared memory to register copy
//   using s2r_copy_op = SM75_U32x4_LDSM_N;
  using s2r_copy_op = SM75_U32x4_LDSM_N_E4M3_V1;
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < M*K; ++i){
      Aptr_host[i] = static_cast<float_e4m3_t>(i);
  }
  for (int i = 0; i < N*K; ++i){
      Bptr_host[i] = static_cast<float_e4m3_t>(i);
  }

  checkCudaStatus(cudaMalloc(&Aptr, sizeof(T) * M * K));
  checkCudaStatus(cudaMalloc(&Bptr, sizeof(T) * N * K));
  checkCudaStatus(cudaMalloc(&Dptr, sizeof(T) * M * N));
